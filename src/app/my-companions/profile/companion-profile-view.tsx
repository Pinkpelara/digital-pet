...
export function CompanionProfileView() {
  ...
  // Living together: while this page is open we notice what they get up to.
  // Discovery is gradual and driven by the hidden seed, never by a purchase.
  useEffect(() => {
    if (!instance) return;
  ...
  if (!instance) {
    return (
      <div className="mx-auto max-w-2xl px-5 py-16">
        ...
        <Link href="/companions" className="btn btn-primary mt-6">
          Meet the companions
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-12 md:px-10">
      <div className="grid gap-8 lg:grid-cols-[1fr_1fr]">
        ...
        <div>
          <p className="kicker">{speciesName(instance)}</p>
          ...
          {revealed && (
            <p className="pop-in mt-4 rounded-2xl bg-moss/12 px-4 py-3 text-ink">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href={studioHref(instance.id)} className="btn btn-primary">
              Customize {instance.name}
            </Link>
            <Link href={liveHref(instance.id)} className="btn btn-ghost">
