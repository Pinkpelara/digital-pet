...
| `companion_instances` | none | select + update name/stats | insert when a companion SKU is granted |
...
4. Customization (outfits) is the one client write: `equipped_items` for instances the user already owns.
